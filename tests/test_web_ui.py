...
class WebUITests(unittest.TestCase):
    ...
    def test_run_audit_job_supports_direct_site_input(self) -> None:
        ...
        mock_audit_domains.assert_called_once_with(
            ...
            mode="audit_light",
            max_pages=20,
            max_total_seconds_per_domain=None,
            delay=0.2,
            crawl_source="mixed",
            respect_robots=True,
            html_output=None,
    ...
    def test_render_file_page_for_audit_json_has_client_facing_report_ui(self) -> None:
        ...
        self.assertIn("Synthèse exécutive", page)
        self.assertNotIn("Votre position face à la concurrence", page)
        self.assertIn("score-gauge", page)
        self.assertIn("Signal principal", page)
        self.assertIn("30 pages analysées", page)
        self.assertIn("Contenus utiles", page)
        self.assertIn("Niveau d'urgence", page)
        ...
        self.assertIn("Quick wins", page)
        self.assertNotIn("Optimisations simples", page)
        self.assertIn("Pages à revoir en priorité", page)
        self.assertIn("Pourquoi elle ressort", page)
        self.assertIn("1 à 2h de travail éditorial", page)
        self.assertIn("Effort :", page)
        self.assertIn("Impact :", page)
        self.assertIn("Angle possible", page)
        ...
        self.assertIn("metric-muted", page)
        self.assertIn("date-titre", page)
        self.assertIn("modérée", page)
